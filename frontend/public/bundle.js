...
(function () {
  ...
  var API_SERVER = window.location.hostname === 'localhost' ? 'http://localhost:3000/' : 'https://api.articl.net/';

  function getAPICategoryNames() {
    return _getAPICategoryNames.apply(this, arguments);
  }

  function _getAPICategoryNames() {
    _getAPICategoryNames = asyncToGenerator(
    /*#__PURE__*/
          ...
          switch (_context.prev = _context.next) {
            case 0:
              return _context.abrupt("return", $.ajax({
                type: 'GET',
                url: "".concat(API_SERVER, "api/v1/categories"),
                dataType: 'json',
                success: function success(data) {
                  return data;
                },
                failure: function failure(error) {
                  throw new Error("Getting categories failed! The server said: ".concat(error));
                }
              }));
  ...
  function getCategoryNames() {
    return _getCategoryNames.apply(this, arguments);
  }

  function _getCategoryNames() {
    _getCategoryNames = asyncToGenerator(
    /*#__PURE__*/
  ...
  function enableOtherSections(enable) {
    $('#sectionOne').find('button').prop('disabled', !enable);
  ...
  function isCategoryPassed() {
    ...
    return categoryNames$1.indexOf($('#mcqCategory').val()) > -1 && $('#mcqCategory').val().length > 0;
  }
  ...
  function writeSuccess(obj) {
    ...
    var questionPreview = $('<div/>').append($('<p/>', {
      html: '<strong>Question created.</strong>'
    })).append($('<p/>', {
      text: "".concat(obj.question.question, " ")
    }));
    var answersPreview = $('<ul/>');
    var correct = $('<i/>', {
    ...
    function addAnswer(answer) {
      var text = $('<p/>', {
        text: "".concat(answer.answer, " ")
      });
      var glyph = answer.correct ? correct : incorrect;
      glyph.appendTo(text);
      var explanation = $('<p/>', {
    ...
    $('#postQuestionSuccess').append(questionPreview); // $('#postQuestionSuccess').append($(`<p>${JSON.stringify(obj)}</p>`));
  }
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              try {
                ...
                $.ajax({
                  type: 'POST',
                  url: "".concat(API_SERVER, "api/v1/questions"),
                  dataType: 'json',
                  cache: false,
  ...
  function enableOtherSections$1(enable) {
    ...
    $('#sectionTwo').find('button:first').prop('disabled', !enable);
    $('#sectionThree').find('button').prop('disabled', !enable);
  }
  ...
  function createSelectCorrectAnswer() {
    for (var i = numberOfExplanationsCounter; i < numberOfAnswersCounter; i += 1) {
      if ($('#answers').find('input')[i].value !== '') {
        $('#correctAnswer').append($('<option>').attr('value', $('#answers').find('input')[i].value).text($('#answers').find('input')[i].value));
      }
    }
  }
  ...
  function createExplanationField(answerNumber, parentElement, required) {
    var textAreaId = "explanation".concat(answerNumber);
    var answerResponse = $("<textarea></textarea>").addClass('md-textarea form-control explanation').prop('required', required).prop('rows', 1).prop('placeholder', 'Type what you would like to display when this answer is selected').prop('id', textAreaId);
    ...
    var explanationFeedback = $('<div/>').addClass('form-text text-danger').prop('id', "explanation".concat(answerNumber, "Feedback"));
    parentElement.append(answerResponse).append(explanationFeedback);
  }

  function createLabel(text, classes, uniqueName, parentElement) {
    var answerLabel = $('<label></label>').addClass(classes).text(text).prop('id', uniqueName);
  ...
  function checkMCQExplnation() {
    if ($(this).val().length < 5) {
      enableOtherSections$1(false);
      markInvalid($(this));
      $("".concat($(this).prop('id'), "Feedback")).text('Explanations must be at least five characters long.');
      return false;
    }

    markValid($(this));
    $("".concat($(this).prop('id'), "Feedback")).text('');
    return true;
  }
  ...
  function enableOtherSections$2(enable) {
    $('#sectionOne').find('button:first').prop('disabled', !enable);
    $('#sectionTwo').find('button').prop('disabled', !enable);
    $('#sectionThree').find('button:first').prop('disabled', !enable);
  }
  ...
  function checkMCQDuplicate(element) {
    ...
    $('#answers input').each(function check() {
      if (JSON.stringify(element) !== JSON.stringify($(this)) && element.val().toLowerCase().trim() === $(this).val().toLowerCase().trim()) {
        markInvalid($(this));
        markInvalid(element);
        $("#".concat(element.prop('id'), "Feedback")).text('Please enter a unique answer.');
        passed = false;
        return false;
      }
    ...
    if (passed) {
      markValid(element);
      $("#".concat(element.prop('id'), "Feedback")).text('');
      return true;
    }
  ...
  function checkMCQAnswer() {
    if ($(this).val().length < 1) {
      markInvalid($(this));
      $("#".concat($(this).prop('id'), "Feedback")).text('Answers must be at least one character long.');
      return false;
    }
  ...
  $('#answers input').on('blur change focus keyup', checkAllFields$2);
